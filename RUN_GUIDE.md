...
| `GRAPH_MODELS` | 그래프/바이파르타이트 단계에서 사용할 GNN 목록. 예: `GCN,GIN,GAT` |
...
| `REG_DATASETS` | 회귀 스크립트가 순회할 데이터셋 subset (`hypergraph,bipartite,clique` 중). 미지정 시 전부 실행 |
...
| 데이터셋 키 | 파일 | 기본 모델 |
| --- | --- | --- |
| `hypergraph` | `regression_hypergraph_dataset.pt` | HyperTransformer, HyperGT, DPHGNN, HJRL, SheafHyperGNN, HyperND, EHNN/ED-HNN, AllSetformer, AllDeepSets, TFHNN, PhenomNN (`HYP_MODELS`) |
| `bipartite` | `regression_bipartite_dataset.pt` | LaplacianPositionalTransformer + GCN, GIN, GAT (`GRAPH_MODELS`) |
| `clique` | `regression_clique_dataset.pt` | LaplacianPositionalTransformer + GCN, GIN, GAT (`GRAPH_MODELS`) |
...
REG_DATASETS=hypergraph,clique \
REG_BATCH_SIZE=8 \
REG_USE_LOG1P=1 \
REG_MAX_SAMPLES=512 \
...
설명:
- `REG_DATASETS`로 특정 파이프라인만 선택(순서도 지정 가능). 값이 없으면 하이퍼그래프→바이파르타이트→클릭 순으로 모두 실행.
...
- `GRAPH_MODELS`가 없으면 GCN·GIN·GAT 세 모델 모두 자동으로 실행됩니다.
